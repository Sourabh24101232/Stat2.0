                    ...
                    <form onSubmit={onSubmitHandler} className="flex w-full max-w-sm flex-col items-center justify-center">
                        ...
                        {isSignUp && (
                            <div className="mb-6 flex h-12 w-full items-center gap-2 overflow-hidden rounded-full border border-gray-300/60 bg-transparent pl-6 dark:border-gray-700">
                                ...
                                <input type="text" value={name} onChange={(event) => setName(event.target.value)} placeholder="Full name" autoComplete="name" className="h-full w-full bg-transparent text-sm text-gray-500/80 outline-none placeholder-gray-500/80 dark:text-gray-200 dark:placeholder-gray-400" required />
                            </div>
                        )}
                        ...
                        <div className="flex h-12 w-full items-center gap-2 overflow-hidden rounded-full border border-gray-300/60 bg-transparent pl-6 dark:border-gray-700">
                            ...
                            <input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Email id" autoComplete="email" className="h-full w-full bg-transparent text-sm text-gray-500/80 outline-none placeholder-gray-500/80 dark:text-gray-200 dark:placeholder-gray-400" required />
                        </div>
                        ...
                        {!isResetPassword && !isVerifyEmail && (
                            <>
                                ...
                                <div className="mt-6 flex h-12 w-full items-center gap-2 overflow-hidden rounded-full border border-gray-300/60 bg-transparent px-6 dark:border-gray-700">
                                    ...
                                    <input type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} placeholder={isSetNewPassword ? 'New password (minimum 8 characters)' : 'Password'} autoComplete={isSignUp || isSetNewPassword ? 'new-password' : 'current-password'} minLength={isSignUp || isSetNewPassword ? 8 : undefined} className="h-full w-full bg-transparent text-sm text-gray-500/80 outline-none placeholder-gray-500/80 dark:text-gray-200 dark:placeholder-gray-400" required />
                                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="flex h-12 w-12 shrink-0 items-center justify-center" aria-label={showPassword ? 'Hide password' : 'Show password'}>
                        ...
                        <p className="mt-4 text-sm text-gray-500/90 dark:text-gray-400">
                            {isVerifyEmail || isSetNewPassword || isResetPassword ? 'Return to login?' : isSignUp ? 'Already have an account?' : "Don't have an account?"}
                            <button
                                type="button"
                                onClick={() => {
                            ...
                            >
                                {isSignUp || isResetPassword || isVerifyEmail || isSetNewPassword ? 'Login' : 'Sign up'}
                            </button>
                        </p>
