    ...
    if (userExists) {
      if (!userExists.isVerified && userExists.authProvider === "local") {
        return res.json({
          ...
          message:
            "Account already registered. Verify your email or request a new OTP.",
        });
      }
...
export const resendOtp = async (req, res) => {
  try {
    ...
    if (user.authProvider === "google") {
      return res.json({
        success: false,
        message: "This Google account does not require email verification",
      });
    }
  ...
  try {
    ...
    if (!email || !otp || !password || password.length < 8) {
      return res.json({
        success: false,
        message:
          "Email, OTP and a password of at least 8 characters are required",
      });
    }
    ...
    if (
      ...
      !(await isOtpMatch(otp, user.resetOtp))
    ) {
      return res.json({ success: false, message: "Invalid or expired OTP" });
    }
...
export const googleAuth = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.json({
        success: false,
        message: "Google credential is not for this app",
      });
    }
